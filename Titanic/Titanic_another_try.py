# -*- coding: utf-8 -*-
"""
Created on Wed Nov 25 15:19:43 2020
...
train_df.info()
train_df.describe()
feature_name = ['Pclass', 'Age', 'Fare', 'Embarked', 'Title', 'SibSp', 'Parch']
max(max(train_df.SibSp), max(test_df.SibSp))


train_df['Pclass'] = train_df['Pclass'].astype(float)
train_df['Title'] = train_df['Title'].astype(float)
train_df['SibSp'] = train_df['SibSp'].astype(float)
train_df['Parch'] = train_df['Parch'].astype(float)
...
for dataset in combine:
    for name in feature_name:
        dataset[name] = dataset[name]/max(max(train_df[name]),max(test_df[name]))
...
history = model.fit(X_train_array, y_train_array, epochs=10, batch_size = 1, 
...
submission.to_csv('submission_Keras3.csv', index=False)
